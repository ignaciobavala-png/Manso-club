...
const LOGOS = [
  { src: '/marcas/gin.png', alt: 'Gin', scale: 'scale-[60%]' },
  { src: '/marcas/settle_transparent_clean.png', alt: 'Settle' },
  { src: '/marcas/takenos_transparent_clean(1).png', alt: 'Takenos' },
  { src: '/marcas/warsteiner_transparent.png', alt: 'Warsteiner', scale: 'scale-125' },
];
          ...
          {LOGOS.map((logo, i) => (
            <div key={i} className="flex items-center justify-center h-20 sm:h-24 md:h-28 overflow-visible">
              <Image
                ...
                width={800}
                height={400}
                className={`w-auto object-contain brightness-0 invert opacity-100 transition-opacity duration-500 h-[200%] ${logo.scale || ''}`}
              />
            </div>
          ))}
        </div>
