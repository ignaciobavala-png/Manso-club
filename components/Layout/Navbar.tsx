      ...
      <nav className={`fixed top-0 w-full z-40 transition-all duration-500 ${
        ...
        <div className="max-w-[1600px] mx-auto px-8 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-10">
            ...
            {/* Streaming — solo si está logueado */}
            {hasMounted && user && (
              <Link
                href="/streaming"
            ...
            {hasMounted && !userLoading && (
              <>
                {user ? (
                  ...
                  >
                    <span className="w-5 h-5 rounded-full bg-manso-terra text-white text-[9px] font-black flex items-center justify-center flex-shrink-0">
                      {userInitial}
                    </span>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 w-full h-full bg-white z-[9999] flex flex-col" style={{ touchAction: 'none' }}>
          ...
          <div className="flex-1 flex flex-col p-8 gap-6 overflow-y-auto">
              ...
              user ? (
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-3 px-4 py-3 bg-zinc-50 rounded-2xl">
                    <span className="w-8 h-8 rounded-full bg-manso-terra text-white text-sm font-black flex items-center justify-center">
                      {userInitial}
                    </span>
                    <div>
            ...
            {/* Streaming mobile — solo si está logueado */}
            {hasMounted && user && (
