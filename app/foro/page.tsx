...
export default async function ForoPage({
  ...
  const [{ data: categorias }, categoriasResult] = await Promise.all([
    supabase
  ...
  let threadsQuery = supabase
    .from('foro_threads')
    .select('id, titulo, categoria_id, autor_nombre, views, reply_count, pinned, cerrado, created_at, updated_at')
      ...
      <div className="relative z-10 max-w-[1000px] mx-auto px-4 md:px-8 pt-32 pb-20">
        ...
        {threadsConCategoria.length === 0 ? (
          <div className="text-center py-20 text-manso-cream/30 text-sm">
            {categoriaSlug ? 'No hay threads en esta categoría todavía.' : 'El foro está vacío por ahora.'}
          </div>
        ) : (
